# sagor-fca — Documentation

Comprehensive reference for **version 4.x**. The library is written in TypeScript; the published package ships `dist/` only. Source under `src/` is on [GitHub](https://github.com/habibuslam5-a11y/sagor-fca).
...
```bash
git clone https://github.com/habibuslam5-a11y/sagor-fca.git
cd fca-unofficial
npm install
npm run build
```
...
```javascript
const { loginLegacy } = require("sagor-fca");
...
```typescript
import { createFcaClient } from "sagor-fca";
...
```typescript
import { createMessengerBot } from "sagor-fca";
...
```typescript
import { attachThreadInfoRealtimeSync } from "sagor-fca";
...
All public exports from `sagor-fca`:
